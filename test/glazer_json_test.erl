...
scalars_test_() ->
  ...
  [?_assertEqual(V, glazer:json_decode(glazer:json_encode(V))) || {V, _} <- Tests]
  ++
  [?_assertEqual(V, glazer:json_decode(J)) || {V, J} <- Tests].

array_test_() ->
  [
    ?_assertEqual([],             glazer:json_decode(<<"[]">>)),
    ?_assertEqual([1, 2, 3],      glazer:json_decode(<<"[1,2,3]">>)),
    ?_assertEqual([[1], [2, 3]],  glazer:json_decode(<<"[[1],[2,3]]">>)),
    ?_assertEqual(<<"[1,2,3]">>,  glazer:json_encode([1, 2, 3]))
  ].

map_test_() ->
  [
    ?_assertEqual(#{},                    glazer:json_decode(<<"{}">>)),
    ?_assertEqual(#{<<"a">> => 1},        glazer:json_decode(<<"{\"a\":1}">>)),
    ?_assertEqual(#{<<"a">> => <<"b">>},  glazer:json_decode(<<"{\"a\":\"b\"}">>)),
    ?_assertEqual(#{<<"a">> => true},     glazer:json_decode(<<"{\"a\":true}">>)),
    ?_assertEqual(#{<<"a">> => null},     glazer:json_decode(<<"{\"a\":null}">>))
  ].

object_as_tuple_test_() ->
  [
    ?_assertEqual({[]},               glazer:json_decode(<<"{}">>,        [object_as_tuple])),
    ?_assertEqual({[{<<"a">>, 1}]},   glazer:json_decode(<<"{\"a\":1}">>, [object_as_tuple])),
    ?_assertEqual({[{<<"x">>, true}, {<<"y">>, false}]},
                  glazer:json_decode(<<"{\"x\":true,\"y\":false}">>, [object_as_tuple]))
  ].
...
duplicate_keys_test_() ->
  [
    ?_assertEqual(#{<<"a">> => 2},             glazer:json_decode(<<"{\"a\":1,\"a\":2}">>)),
    ?_assertEqual(#{<<"a">> => 3, <<"b">> => 2}, glazer:json_decode(<<"{\"a\":1,\"b\":2,\"a\":3}">>)),
    ?_assertEqual(#{a => 2},                   glazer:json_decode(<<"{\"a\":1,\"a\":2}">>, [{keys, atom}])),
    ?_assertEqual(#{a => 2},                   glazer:json_decode(<<"{\"a\":1,\"a\":2}">>, [{keys, existing_atom}])),

    %% duplicates inside a nested object are deduped too
    ?_assertEqual(#{<<"a">> => #{<<"x">> => 2}}, glazer:json_decode(<<"{\"a\":{\"x\":1,\"x\":2}}">>)),
    ...
    ?_assertEqual({[{<<"a">>, 1}, {<<"a">>, 2}]},
                  glazer:json_decode(<<"{\"a\":1,\"a\":2}">>, [object_as_tuple])),
    ?_assertEqual({[{<<"a">>, 1}, {<<"b">>, 2}, {<<"a">>, 3}]},
                  glazer:json_decode(<<"{\"a\":1,\"b\":2,\"a\":3}">>, [object_as_tuple])),
    ?_assertEqual({[{<<"foo">>, {[{<<"bar">>, 1}, {<<"bar">>, 2}]}}]},
                  glazer:json_decode(<<"{\"foo\":{\"bar\":1,\"bar\":2}}">>, [object_as_tuple]))
  ].
...
dedupe_keys_test_() ->
  [
    ...
    ?_assertEqual({[{<<"foo">>, 1}]},
                  glazer:json_decode(<<"{\"foo\":1}">>, [object_as_tuple, dedupe_keys])),

    %% Basic dedup: last value wins
    ?_assertEqual({[{<<"foo">>, 2}]},
                  glazer:json_decode(<<"{\"foo\":1,\"foo\":2}">>, [object_as_tuple, dedupe_keys])),

    %% Multiple repeats: last value wins
    ?_assertEqual({[{<<"foo">>, 3}]},
                  glazer:json_decode(<<"{\"foo\":1,\"foo\":2,\"foo\":3}">>, [object_as_tuple, dedupe_keys])),

    %% Sub-objects are deduped too
    ?_assertEqual({[{<<"foo">>, {[{<<"bar">>, 2}]}}]},
                  glazer:json_decode(<<"{\"foo\":{\"bar\":1,\"bar\":2}}">>, [object_as_tuple, dedupe_keys])),
    ...
    ?_assertEqual({[{<<"bar">>, 1}, {<<"baz">>, 3}, {<<"foo">>, 4}]},
                  glazer:json_decode(<<"{\"bar\":1,\"foo\":2,\"baz\":3,\"foo\":4}">>,
                                 [object_as_tuple, dedupe_keys]))
  ].

null_atom_test_() ->
  [
    ?_assertEqual(nil,       glazer:json_decode(<<"null">>, [use_nil])),
    ?_assertEqual(my_nil,    glazer:json_decode(<<"null">>, [{null_term, my_nil}])),
    ?_assertEqual(<<"null">>,glazer:json_encode(nil,  [use_nil])),
    ?_assertEqual(<<"null">>,glazer:json_encode(null, []))
  ].

encode_map_keys_test_() ->
  [
    %% atom keys
    ?_assertMatch(<<"{", _/binary>>, glazer:json_encode(#{a => 1})),
    %% binary keys
    ?_assertMatch(<<"{", _/binary>>, glazer:json_encode(#{<<"a">> => 1}))
  ].
...
pretty_test_() ->
  Compact = <<"[1,2,3]">>,
  Pretty = glazer:json_prettify(Compact),
  [
    ?_assertMatch(<<"[\n", _/binary>>, Pretty),
    ?_assertEqual(Compact, glazer:json_minify(Pretty))
  ].
...
nested_test_() ->
  ...
  [
    ?_assertEqual(Expected,   glazer:json_decode(JSON)),
    ?_assertEqual(Expected,   glazer:json_decode(glazer:json_encode(Expected)))
  ].
...
large_integer_test_() ->
  ...
  [
    ?_assertEqual(Big, glazer:json_decode(integer_to_binary(Big)))
  ].
...
scan_complete_test_() ->
  [
    ?_assertEqual({complete, 7},  glazer:json_scan(<<"{\"a\":1}">>)),
    ?_assertEqual({complete, 7},  glazer:json_scan(<<"{\"a\":1} {\"b\":2}">>)),
    ?_assertEqual({complete, 2},  glazer:json_scan(<<"[]">>)),
    ?_assertEqual({complete, 13}, glazer:json_scan(<<"[1,2,[3,4],5]rest">>))
  ].

scan_incomplete_test_() ->
  [
    ?_assertMatch({incomplete, _}, glazer:json_scan(<<"{\"a\":">>)),
    ?_assertMatch({incomplete, _}, glazer:json_scan(<<"[1,2">>)),
    ?_assertMatch({incomplete, _}, glazer:json_scan(<<"123">>)),
    ?_assertMatch({incomplete, _}, glazer:json_scan(<<"\"unterminated">>))
  ].
...
scan_resume_test_() ->
  [
    ?_test(begin
      Part1 = <<"{\"a\":">>,
      Part2 = <<"1}">>,
      {incomplete, S1} = glazer:json_scan(Part1),
      ?assertEqual({complete, 7}, glazer:json_scan(<<Part1/binary, Part2/binary>>, S1))
    end),
    ...
    ?_test(begin
      Chunk1 = <<"{\"k\":\"ab\\">>,
      Chunk2 = <<"\"cd\"}">>,
      {incomplete, S2} = glazer:json_scan(Chunk1),
      Whole = <<Chunk1/binary, Chunk2/binary>>,
      ?assertEqual({complete, byte_size(Whole)}, glazer:json_scan(Whole, S2))
    end),
    ...
    ?_test(begin
      Part1 = <<"{\"a\":{\"b\":">>,
      Part2 = <<"[1,2]}}">>,
      {incomplete, S3} = glazer:json_scan(Part1),
      Whole = <<Part1/binary, Part2/binary>>,
      ?assertEqual({complete, byte_size(Whole)}, glazer:json_scan(Whole, S3))
    end)
  ].
...
json_stream_feed_basic_test_() ->
  [
    ...
    ?_test(begin
      D0 = glazer:json_stream_decoder(),
      {[], D1} = glazer:json_stream_feed(D0, <<"{\"a\":">>),
      {Vals, _D2} = glazer:json_stream_feed(D1, <<"1}">>),
    ...
    ?_test(begin
      D0 = glazer:json_stream_decoder(),
      {Vals, _D1} = glazer:json_stream_feed(D0, <<"{\"a\":1} {\"b\":2} {\"c\":3}">>),
    ...
    ?_test(begin
      D0 = glazer:json_stream_decoder(),
      {V1, D1} = glazer:json_stream_feed(D0, <<"{\"a\":1} {\"b\":">>),
      {V2, _D2} = glazer:json_stream_feed(D1, <<"2}">>),
      ?assertEqual([#{<<"a">> => 1}], V1),
      ?assertEqual([#{<<"b">> => 2}], V2)
    end)
  ].
...
json_stream_feed_ndjson_test_() ->
  ?_test(begin
    ...
    {Vals, DLast} = lists:foldl(
      fun(Byte, {Acc, D}) ->
        {V, D2} = glazer:json_stream_feed(D, <<Byte>>),
        {Acc ++ V, D2}
      end, {[], glazer:json_stream_decoder()}, binary_to_list(Doc)),
    ?assertEqual({ok, []}, glazer:json_stream_eof(DLast)),
    ?assertEqual([#{<<"x">> => 1},
                  #{<<"y">> => [1, 2, 3]},
                  #{<<"z">> => <<"hi">>}], Vals)
  end).
...
json_stream_eof_test_() ->
  [
    ...
    ?_test(begin
      D0 = glazer:json_stream_decoder(),
      {[], D1} = glazer:json_stream_feed(D0, <<"   42">>),
      ?assertEqual({ok, [42]}, glazer:json_stream_eof(D1))
    end),

    %% trailing whitespace at EOF yields no extra value
    ?_test(begin
      D0 = glazer:json_stream_decoder(),
      {[#{<<"a">> := 1}], D1} = glazer:json_stream_feed(D0, <<"{\"a\":1}\n">>),
      ?assertEqual({ok, []}, glazer:json_stream_eof(D1))
    end),
    ...
    ?_test(begin
      D0 = glazer:json_stream_decoder(),
      {[], D1} = glazer:json_stream_feed(D0, <<"{\"a\":">>),
      ?assertMatch({error, _}, glazer:json_stream_eof(D1))
    end)
  ].

json_stream_decoder_opts_test_() ->
  ?_test(begin
    D0 = glazer:json_stream_decoder([{keys, atom}]),
    {[#{a := 1}], _D1} = glazer:json_stream_feed(D0, <<"{\"a\":1}">>)
  end).

%% Random chunk boundaries shouldn't change the decoded result.
stream_random_split_test_() ->
  {timeout, 30, ?_test(begin
    Lines = [glazer:json_encode(#{<<"i">> => I, <<"v">> => lists:seq(1, I rem 7)})
             || I <- lists:seq(1, 100)],
    Doc = iolist_to_binary([[L, <<"\n">>] || L <- Lines]),
    Expected = [glazer:json_decode(L) || L <- Lines],
    ...
    lists:foreach(fun(_) ->
      ...
      {Vals, DLast} = lists:foldl(
        fun(Piece, {Acc, D}) ->
          {V, D2} = glazer:json_stream_feed(D, Piece),
          {Acc ++ V, D2}
        end, {[], D0}, Pieces),
      {ok, Trailing} = glazer:json_stream_eof(DLast),
...
rfc8259_literals_test_() ->
  [
    ?_assertEqual(true,  glazer:json_decode(<<"true">>)),
    ?_assertEqual(false, glazer:json_decode(<<"false">>)),
    ?_assertEqual(null,  glazer:json_decode(<<"null">>)),

    ?_assertError({parse_error, _}, glazer:json_decode(<<"True">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"TRUE">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"NULL">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"nul">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"tru">>))
  ].
...
rfc8259_numbers_test_() ->
  [
    ...
    ?_assertEqual(0,     glazer:json_decode(<<"0">>)),
    ?_assertEqual(42,    glazer:json_decode(<<"42">>)),
    ?_assertEqual(-7,    glazer:json_decode(<<"-7">>)),
    ?_assertEqual(3.14,  glazer:json_decode(<<"3.14">>)),
    ?_assertEqual(1.0e10, glazer:json_decode(<<"1e10">>)),
    ?_assertEqual(1.0e-10, glazer:json_decode(<<"1e-10">>)),
    ?_assertEqual(1.5e3, glazer:json_decode(<<"1.5E+3">>)),

    %% RFC 8259 forbids a leading '+' on numbers
    ?_assertError({parse_error, _}, glazer:json_decode(<<"+1">>)),

    %% RFC 8259 requires at least one digit before '.', and requires a digit
    %% after '.'; a bare leading '.' is not a valid number
    ?_assertError({parse_error, _}, glazer:json_decode(<<".5">>)),

    %% --- Lenient deviations from strict RFC 8259 grammar ---

    %% RFC 8259's `int` production forbids leading zeros (e.g. "01"), but
    %% glazer accepts them.
    ?_assertEqual(1,   glazer:json_decode(<<"01">>)),
    ?_assertEqual(-1,  glazer:json_decode(<<"-01">>)),
...
rfc8259_strings_test_() ->
  [
    ?_assertEqual(<<"hello">>,  glazer:json_decode(<<"\"hello\"">>)),
    ?_assertEqual(<<"">>,       glazer:json_decode(<<"\"\"">>)),
    ...
    ?_assertEqual(<<"\\">>,     glazer:json_decode(<<"\"\\\\\"">>)),
    ?_assertEqual(<<"/">>,      glazer:json_decode(<<"\"\\/\"">>)),
    ?_assertEqual(<<"\b">>,     glazer:json_decode(<<"\"\\b\"">>)),
    ?_assertEqual(<<"\f">>,     glazer:json_decode(<<"\"\\f\"">>)),
    ?_assertEqual(<<"\n">>,     glazer:json_decode(<<"\"\\n\"">>)),
    ?_assertEqual(<<"\r">>,     glazer:json_decode(<<"\"\\r\"">>)),
    ?_assertEqual(<<"\t">>,     glazer:json_decode(<<"\"\\t\"">>)),

    %% \uXXXX escapes, including a surrogate pair combined into one
    %% UTF-8-encoded code point
    ?_assertEqual(<<"A">>,      glazer:json_decode(<<"\"\\u0041\"">>)),
    ?_assertEqual(<<240,159,152,128>>,
                  glazer:json_decode(<<"\"\\ud83d\\ude00\"">>)),

    %% an unterminated string is rejected
    ?_assertError({parse_error, _}, glazer:json_decode(<<"\"unterminated">>)),
    ...
    ?_assertEqual(<<"x41">>,    glazer:json_decode(<<"\"\\x41\"">>)),
    ...
    %% rather than rejecting it.
    ?_assertEqual(<<237,160,128>>, glazer:json_decode(<<"\"\\ud800\"">>)),
    ?_assertEqual(<<237,176,128>>, glazer:json_decode(<<"\"\\udc00\"">>))
  ].
...
rfc8259_structure_test_() ->
  [
    ...
    ?_assertError({parse_error, _}, glazer:json_decode(<<"[1,2,]">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"{\"a\":1,}">>)),
    ...
    ?_assertError({parse_error, _}, glazer:json_decode(<<"{'a':1}">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"{a:1}">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"{\"a\":1 // comment\n}">>))
  ].
...
rfc8259_top_level_test_() ->
  [
    ?_assertEqual(<<"hi">>, glazer:json_decode(<<"\"hi\"">>)),
    ?_assertEqual(42,       glazer:json_decode(<<"42">>)),
    ?_assertEqual(true,     glazer:json_decode(<<"true">>)),
    ?_assertEqual(false,    glazer:json_decode(<<"false">>)),
    ?_assertEqual(null,     glazer:json_decode(<<"null">>)),
    ...
    ?_assertError({parse_error, _}, glazer:json_decode(<<"">>)),

    %% trailing whitespace after the value is allowed
    ?_assertEqual(1, glazer:json_decode(<<"1   ">>)),
    ?_assertEqual(1, glazer:json_decode(<<"  1  \t\r\n">>)),

    %% trailing non-whitespace data after a complete value is rejected
    ?_assertError({parse_error, _}, glazer:json_decode(<<"1 2">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"123abc">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"[1] [2]">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"{} {}">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"true false">>)),
...
rfc8259_unterminated_test_() ->
  [
    ...
    ?_assertError({parse_error, _}, glazer:json_decode(<<"[1,">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"{\"a\":1,">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"[\"a\",\n4\n,1,">>)),
...
max_depth_test_() ->
  [
    ...
    ?_test(begin
      ...
      {ok, _} = glazer:json_try_decode(Bin)
    end),
...
jsontestsuite_implementation_defined_test_() ->
  [
    ...
    ?_assertEqual([100000000000000000000],
                  glazer:json_decode(<<"[100000000000000000000]">>)),
    ?_assertEqual([-237462374673276894279832749832423479823246327846],
                  glazer:json_decode(<<"[-237462374673276894279832749832423479823246327846]">>)),

    %% Numbers with an exponent so large/small that the magnitude over- or
    %% underflows a double are rejected.
    ?_assertError({parse_error, _}, glazer:json_decode(<<"[1e999]">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"[-1e999]">>)),
    ?_assertError({parse_error, _}, glazer:json_decode(<<"[1e-999]">>)),

    %% A byte-order mark (BOM) is not stripped or otherwise recognised; JSON
    %% text must be UTF-8 without a BOM.
    ?_assertError({parse_error, _}, glazer:json_decode(<<239,187,191, "{}">>)),
    ...
    ?_assertEqual([<<255>>],          glazer:json_decode(<<"[\"", 255, "\"]">>)),
    ?_assertEqual([<<129>>],          glazer:json_decode(<<"[\"", 129, "\"]">>)),
    ...
    ?_test(begin
      ...
      ?assertMatch({ok, _}, glazer:json_try_decode(Bin))
    end)
  ].
